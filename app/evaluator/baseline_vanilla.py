...
# hasil_desk_gemini_25_pro = {
#     "zmapsdk.api.APIServer" : """
# Class ini menyederhanakan proses memulai server Uvicorn yang melayani aplikasi FastAPI.
#     """,
#     "zmapsdk.core.ZMap" : """
# Class `ZMap` mengabstraksi interaksi langsung dengan executable ZMap. Ini menyediakan metode yang mudah digunakan untuk melakukan scan, mendapatkan informasi tentang ZMap (seperti versi, modul yang tersedia), mengelola file input (blocklist, allowlist), dan mem-parsing hasil output.
#     """,
#     "zmapsdk.input.ZMapInput" : """
# Class ini bertanggung jawab untuk menangani target scan, baik dalam bentuk subnet yang ditambahkan secara programatik maupun dari file. Ia juga mengelola file blocklist dan allowlist, termasuk validasi file dan pembuatan file baru dari daftar subnet/IP.
#     """,
#     "zmapsdk.runner.ZMapRunner" : """
# `ZMapRunner` adalah jembatan antara Python SDK dan executable ZMap. Ia menangani penerjemahan objek konfigurasi Python menjadi argumen command-line, menjalankan proses ZMap, menangkap outputnya, dan menangani error eksekusi.
#     """,
#     "zmapsdk.core.ZMap.scan" : """
# Metode ini adalah cara utama untuk menjalankan scan. Metode ini membuat objek konfigurasi sementara untuk scan saat ini, menetapkan parameter inti seperti port target dan subnet, memproses opsi tambahan melalui `_process_scan_options`, dan akhirnya mendelegasikan eksekusi scan ke `ZMapRunner`.
#     """,
#     "zmapsdk.runner.ZMapRunner.run_command" : """
# Metode ini mengumpulkan semua parameter dari objek-objek konfigurasi dan `kwargs` tambahan, membangun perintah menggunakan `_build_command`, dan kemudian menjalankannya. Jika `callback` disediakan, ia menggunakan `subprocess.Popen` untuk memproses output secara real-time. Jika tidak, ia menggunakan `subprocess.run` yang lebih sederhana untuk menunggu proses selesai dan menangkap semua output sekaligus.
#     """,
#     "zmapsdk.parser.ZMapParser.parse_csv_results" : """
#  Metode ini dapat menangani beberapa skenario:
#     1. Jika `fields` tidak disediakan dan file memiliki header, ia akan menggunakan `csv.DictReader` untuk secara otomatis membuat dictionary.
#     2. Jika `fields` disediakan, ia akan menggunakan `csv.reader` dan memetakan setiap kolom ke nama field yang sesuai.
#     3. Jika file hanya berisi satu kolom (biasanya hanya alamat IP), ia akan mengasumsikan field tersebut adalah `saddr`.
#     Jika terjadi kesalahan parsing, `ZMapParserError` akan dilemparkan.
#     """,
#     "zmapsdk.parser.ZMapParser.stream_results" : """
# Metode ini sangat berguna untuk file hasil yang sangat besar karena tidak memuat seluruh file ke dalam memori. Ia berfungsi sebagai generator, membaca file CSV baris per baris dan menghasilkan dictionary untuk setiap baris. Logikanya mirip dengan `parse_csv_results` tetapi menggunakan `yield` alih-alih membangun daftar di memori.
#     """,
# }
