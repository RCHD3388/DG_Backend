...
from app.services.docgen.agents.prompt.writer_prompt import AVAILABLE_VERSIONS
...
class Writer(BaseAgent):
      ...
      def __init__(self, llm_config: Dict[str, Any], prompt_version = "v2"):
         """Inisialisasi Writer, memuat semua template prompt."""
         super().__init__("writer", llm_config=llm_config)
         
         logger.info_print("Writer version - " + prompt_version)
...
"""
         ...
         selected_prompt = AVAILABLE_VERSIONS.get(prompt_version)
         
         self.rules_for_function_template: str = selected_prompt.function_rules
         self.rules_for_class_template: str = selected_prompt.class_rules
